...
import { DEFAULT_COLOR, ICON_COLORS } from "./constants/colors";
...
export default class Soundboard extends Plugin {
  onload(): void {
    ...
    this.app.workspace.onLayoutReady(async () => {
      ...
      this.registerEvent(
        this.app.vault.on(
          "rename",
          async (file: TAbstractFile, previousPath: string | null) => {
            if (!previousPath) return; // FIXME: when does this happen?

            if (file instanceof TFile)
              await this.onFileRenamed(file, previousPath);
            if (file instanceof TFolder)
              await this.onFolderRenamed(file, previousPath);
          },
        ),
      );
  ...
  async activateView() {
    ...
    if (leaves[0] !== undefined) {
      ...
      if (leaf === null) {
        leaf = await workspace.ensureSideLeaf(VIEW_TYPE_SOUNDBOARD, 'right', { split: false });
      }
  ...
  buildTrack(file: TFile): Track {
    const idx = Math.floor(Math.random() * ICON_COLORS.length);
    const { bg, fg } = ICON_COLORS[idx] ?? DEFAULT_COLOR;
  ...
  buildVaultTracks(savedData: PluginConfiguration): Record<string, Track> {
    ...
    const audioTracks = this.app.vault
      .getAllLoadedFiles()
      ...
      .reduce((tracks, file: TFile) => {
        const track =
          savedData.data.tracks[file.path] !== undefined
            ? {
                ...savedData.data.tracks[file.path],
                uri: vault.adapter.getResourcePath(file.path),
              }
  ...
  async loadConfig() {
    ...
    const res = SoundboardDataSchema.safeParse(loadedData);

    const settings: PluginSettings = res.success
      ? { ...DEFAULT_SETTINGS, ...res.data.settings }
      : { ...DEFAULT_SETTINGS };

    const data: PluginData = res.success
      ? { ...DEFAULT_DATA, ...res.data.data }
      : { ...DEFAULT_DATA };

    appState.settings = settings;
    appState.tracks = this.buildVaultTracks({ settings, data });
  ...
  async onRootFolderChanged(rootFolder: string): Promise<void> {
    ...
    appState.tiles = appState.tiles.filter(
      (tile) => tile.track && !deleted.includes(tile.track),
    );

    appState.sections = appState.sections.map((section) => {
  ...
  renameTrack(file: TFile, previousPath: string): void {
    ...
    appState.tiles = appState.tiles.map((tile) => {
    ...
    // update track
    appState.tracks[file.path] = Object.assign(
      {},
      appState.tracks[previousPath],
    );
  ...
  deleteTrack(file: TFile): void {
    ...
    appState.tiles = appState.tiles.filter((tile) => tile.track !== file.path);
